(function(GeoJSON) {
  ...
  function InvalidGeometryError() {
    ...
    this.message =
      this.message || 'Invalid Geometry: ' + 'item: ' + JSON.stringify(item) + ', params: ' + JSON.stringify(params);
  }
  ...
  GeoJSON.isGeometryValid = function(geometry) {
    ...
    return (
      !!geometry.type && !!geometry.coordinates && Array.isArray(geometry.coordinates) && !!geometry.coordinates.length
    );
  };
